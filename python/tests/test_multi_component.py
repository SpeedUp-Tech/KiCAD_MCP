...
async def run_two_components_workflow() -> str:
    """Create session -> create schematic -> add two components -> export PDF -> close session.
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # 2) create schematic
            tmp_dir = Path(tempfile.mkdtemp(prefix='schematic_two_comp_'))
            schematic_name = 'two_components_demo'
            ...
            # 3) add two different components
            components = [
                {
                    'type': 'R',
                    'reference': 'R1',
...
class SchematicWorkflowTwoComponentsPDFTests(unittest.TestCase):
    def test_two_components_schematic_export_pdf(self) -> None:
        pdf_path = asyncio.run(run_two_components_workflow())
