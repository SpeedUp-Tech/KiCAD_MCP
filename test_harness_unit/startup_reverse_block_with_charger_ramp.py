...
USE_CASE: Dict[str, Any] = {
    "name": "startup_reverse_block_with_charger_ramp",
    "mode": "auto",
    "given": {
        "VIN": {"ramp": {"start_V": 0.0, "stop_V": 4.2, "rise_ms": 10.0, "delay_ms": 0.0}},
        "load": {"type": "battery_model", "voc_V": 3.9, "esr_ohm": 0.05, "capacity_Ah": 2.0},
        "env": {"source_impedance_mohm": 50.0, "pack_esr_mohm": 50.0, "wiring_inductance_nH": 50.0},
        "ambient": {"temp_C": 25.0},
    },
    "when": {"observe_window_ms": [0.0, 12.0]},
}
...
def simulation_harness(use_case: Dict[str, Any] | None = None) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    vbatt.dc_value = float(uc["given"]["load"].get("voc_V", 0.0))
    iload.initial_value = 0.0
    iload.pulsed_value = 0.0
    iload.delay_time = 0.0
